...
export default function App() {
  ...
  const loadAll = async () => {
    setLoading(true);
    const [{ data: u }, { data: m }, { data: b }] = await Promise.all([
  ...
  const placeBet = async (matchId, side, amount) => {
    ...
    const { error: betErr } = await sb.from("bets").insert({ id: `b${Date.now()}`, match_id: matchId, username: currentUser.username, side, amount });
    if (betErr) return false;
    const { error: coinErr } = await sb.from("users").update({ coins: currentUser.coins - amount }).eq("username", currentUser.username);
  ...
  const resolveMatch = async (matchId, winner) => {
    ...
    const winBets = bets.filter(b => b.match_id === matchId && b.side === winner);
    for (const bet of winBets) {
      const user = users[bet.username];
      if (user) {
        await sb.from("users").update({ coins: user.coins + Math.floor(bet.amount * winOdds) }).eq("username", bet.username);
      }
    }
  ...
  if (!currentUser) return (
    <div className="auth-wrap">
      <style>{css}</style>
      <AuthScreen mode={authMode} setMode={setAuthMode} users={users} loadAll={loadAll} setCurrentUser={(u) => { setCurrentUser(u); localStorage.setItem("aoe2_current_user", JSON.stringify(u)); }} />
    </div>
  );
...
function AuthScreen({ mode, setMode, users, loadAll, setCurrentUser }) {
  ...
  const submit = async (e) => {
    ...
    if (mode === "register") {
      ...
      const nu = { username: form.username.trim(), password: form.password, experience: parseInt(form.experience) || 0, coins: INITIAL_COINS, bio: "", joined: Date.now() };
              ...
              <div key={match.id} className="match-card" style={{ animationDelay: `${i * 0.05}s` }}>
                <div className="match-head">
                  ...
                  <div className="flex gap-8">
                    ...
                    <span className={`badge ${match.status === "open" ? "badge-open" : "badge-finished"}`}>{match.status === "open" ? "LIVE" : "KRAJ"}</span>
                  </div>
                </div>
                <div className="match-body">
                  <div className="versus">
                    <div className="team"><div className="team-name">{teamA.map(p => p.username).join(" & ")}</div><div className="team-exp">{teamA.map(p => `${p.experience || 0}g`).join(" & ")} isku.</div></div>
                    <div className="vs-text">VS</div>
                    <div className="team"><div className="team-name">{teamB.map(p => p.username).join(" & ")}</div><div className="team-exp">{teamB.map(p => `${p.experience || 0}g`).join(" & ")} isku.</div></div>
                  </div>
...
function ProfileTab({ currentUser, bets, matches, loadAll, refreshUser }) {
  ...
  const save = async () => {
    await sb.from("users").update({ experience: parseInt(form.experience) || 0, bio: form.bio }).eq("username", currentUser.username);
    await refreshUser(currentUser.username);
