...
:root {
  ...
  --gold:#c8922a; --gold2:#e8b84b; --gold3:#ffd875; --gold-glow:rgba(200,146,42,0.2);
...
.nav-tab{background:none;border:none;cursor:pointer;font-family:'Cinzel',serif;font-size:0.78rem;font-weight:600;color:var(--text2);padding:8px 16px;letter-spacing:1px;border-bottom:2px solid transparent;margin-bottom:-2px;transition:all 0.2s;white-space:nowrap;text-transform:uppercase;}
...
.admin-badge{background:linear-gradient(135deg,var(--red),var(--red2));color:#fff;font-family:'Cinzel',serif;font-size:0.6rem;font-weight:700;padding:3px 8px;border-radius:2px;letter-spacing:2px;border:1px solid rgba(255,255,255,0.1);}
...
.btn-primary{background:linear-gradient(180deg,var(--gold2) 0%,var(--gold) 100%);color:#1a1208;border:1px solid var(--gold3);box-shadow:0 2px 8px rgba(200,146,42,0.3),inset 0 1px 0 rgba(255,255,255,0.15);}
...
.btn-ghost:hover:not(:disabled){color:var(--text);border-color:var(--border2);background:rgba(255,255,255,0.07);}
...
.form-input{width:100%;padding:9px 12px;background:var(--stone3);border:1px solid var(--border);color:var(--text);font-family:'Crimson Pro',serif;font-size:1rem;border-radius:2px;outline:none;transition:border-color 0.15s,box-shadow 0.15s;}
...
.panel-title{font-family:'Cinzel',serif;font-size:0.9rem;font-weight:700;color:var(--gold2);letter-spacing:1px;text-transform:uppercase;}
...
.match-head-title{font-family:'Cinzel',serif;font-size:0.85rem;font-weight:700;color:var(--text);flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;letter-spacing:0.5px;}
...
.team-exp{font-size:0.72rem;color:var(--text3);font-style:italic;}
.vs-text{text-align:center;font-family:'Cinzel',serif;font-weight:900;font-size:0.9rem;color:var(--border2);letter-spacing:1px;}
...
.modal-close{background:rgba(0,0,0,0.3);border:1px solid var(--border);color:var(--text2);width:30px;height:30px;border-radius:2px;cursor:pointer;font-size:1rem;display:flex;align-items:center;justify-content:center;flex-shrink:0;transition:all 0.15s;}
...
.auth-logo p{color:var(--text2);font-size:0.9rem;margin-top:6px;font-style:italic;letter-spacing:1px;}
...
.stat-val{font-family:'Cinzel',serif;font-size:1.8rem;font-weight:900;color:var(--gold2);}
...
.flex{display:flex;} .gap-8{gap:8px;} .divider{height:1px;background:var(--border);margin:16px 0;}
@media(max-width:768px){.nav-tabs{display:none;}.hamburger{display:flex;}.page{padding:16px;}.stats-grid{grid-template-columns:repeat(3,1fr);}}
...
export default function App() {
  ...
  const updateUser = (updated) => { const nu = { ...users, [updated.username]: updated }; saveUsers(nu); saveCurrentUser(updated); };
  ...
  const placeBet = (matchId, side, amount) => {
    ...
    saveBets([...bets, { id: `b${Date.now()}`, matchId, username: currentUser.username, side, amount }]);
    updateUser({ ...currentUser, coins: currentUser.coins - amount });
    return true;
  };
  ...
  const resolveMatch = (matchId, winner) => {
    ...
    bets.filter(b => b.matchId === matchId && b.side === winner).forEach(bet => {
      ...
      if (upd[bet.username]) upd[bet.username] = { ...upd[bet.username], coins: upd[bet.username].coins + Math.floor(bet.amount * odds) };
    });
    saveUsers(upd);
    if (currentUser && upd[currentUser.username]) saveCurrentUser(upd[currentUser.username]);
  ...
  if (loading) return <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#1a1710" }}><style>{css}</style><span style={{ fontFamily: "'Cinzel',serif", color: "#c8922a", letterSpacing: "4px" }}>UČITAVANJE...</span></div>;
  if (!currentUser) return <div className="auth-wrap"><style>{css}</style><AuthScreen mode={authMode} setMode={setAuthMode} users={users} saveUsers={saveUsers} saveCurrentUser={saveCurrentUser} /></div>;

  const tabs = [{ id: "matches", label: "⚔ Mečevi" }, { id: "leaderboard", label: "🏆 Ljestvica" }, { id: "profile", label: "👤 Profil" }];
    ...
    <div className="app">
      ...
      <div className={`mobile-menu ${mobileOpen ? "open" : ""}`}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingTop: 10, borderTop: "1px solid var(--border)", marginTop: 4 }}>
          <span style={{ fontSize: "0.82rem", color: "var(--text2)", fontStyle: "italic" }}>{currentUser.username}</span>
          <button className="btn btn-ghost btn-sm" onClick={logout}>Odjava</button>
        </div>
      </div>
      <div className="page">
...
function AuthScreen({ mode, setMode, users, saveUsers, saveCurrentUser }) {
  ...
  const submit = (e) => {
    ...
    if (mode === "register") {
      ...
      saveUsers({ ...users, [nu.username]: nu }); saveCurrentUser(nu);
    } else {
      ...
      <div className="panel">
        <div className="panel-body">
          ...
          <form onSubmit={submit}>
            ...
            {mode==="register" && <div className="form-group"><label className="form-label">Godine iskustva u AoE2</label><input className="form-input" type="number" min="0" max="30" value={form.experience} onChange={e=>setForm(f=>({...f,experience:e.target.value}))} placeholder="0" /></div>}
            <button className="btn btn-primary btn-lg btn-full" type="submit">{mode==="login"?"⚔ Ulaz na Arenu":"✦ Registriraj se"}</button>
          </form>
...
function MatchesTab({ matches, bets, currentUser, placeBet, resolveMatch, isAdmin, showNewMatch, setShowNewMatch, saveMatches, users }) {
  ...
  const filtered = matches.filter(m => { if(filter==="sve")return true; if(filter==="otvoreno")return m.status==="open"; if(filter==="završeno")return m.status==="finished"; return m.type===filter; });
  const myBet = (id) => bets.find(b=>b.matchId===id&&b.username===currentUser.username);
  ...
  return (
    <div>
      {isAdmin && <div className="admin-bar"><div className="admin-bar-text"><strong>⚡ Admin Panel</strong>Prijavljen si kao administrator. Možeš kreirati mečeve i proglašavati pobjednike.</div><button className="btn btn-primary" onClick={()=>setShowNewMatch(true)}><IcoPlus /> Novi meč</button></div>}
      <div className="section-header">
        <h2 className="section-title">⚔ Mečevi</h2>
        <span style={{fontSize:"0.78rem",color:"var(--text3)",fontStyle:"italic"}}>{filtered.length} {filtered.length===1?"meč":"mečeva"}</span>
      </div>
              ...
              <div key={match.id} className="match-card" style={{animationDelay:`${i*0.05}s`}}>
                ...
                <div className="match-body">
                  <div className="versus">
                    <div className="team"><div className="team-name">{match.teamA.map(p=>p.username).join(" & ")}</div><div className="team-exp">{match.teamA.map(p=>`${p.experience||0}g`).join(" & ")} isku.</div></div>
                    <div className="vs-text">VS</div>
                    <div className="team"><div className="team-name">{match.teamB.map(p=>p.username).join(" & ")}</div><div className="team-exp">{match.teamB.map(p=>`${p.experience||0}g`).join(" & ")} isku.</div></div>
                  </div>
...
function NewMatchModal({ onClose, saveMatches, matches, users }) {
  ...
  const parse=(str)=>str.split(",").map(s=>s.trim()).filter(Boolean).map(name=>{ const u=users[name]; return u?{username:u.username,experience:u.experience||0}:{username:name,experience:0}; });
  const create=()=>{
    ...
    <div className="overlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="modal">
        ...
        <div className="modal-body">
          ...
          <p className="modal-hint">Kvote se računaju prema godinama iskustva. Registrirani igrači automatski prenose iskustvo.</p>
          <div className="flex gap-8"><button className="btn btn-primary" onClick={create}><IcoPlus/> Kreiraj meč</button><button className="btn btn-ghost" onClick={onClose}>Odustani</button></div>
        </div>
      </div>
    </div>
  );
      ...
      <div className="panel"><div className="panel-body">
        {sorted.length===0?<div className="empty-state" style={{padding:"40px 0"}}><p>Nema registriranih igrača.</p></div>:sorted.map((u,i)=>(
          <div key={u.username} className="lb-row">
            ...
            <div className="lb-info"><div className="lb-name">{u.username} {u.username===ADMIN_USERNAME&&<span className="admin-badge">ADMIN</span>}</div><div className="lb-sub">{u.experience||0} god. iskustva</div></div>
...
function ProfileTab({ currentUser, bets, matches, updateUser }) {
  ...
  const save=()=>{ updateUser({...currentUser,experience:parseInt(form.experience)||0,bio:form.bio}); setEditing(false); setSaved(true); setTimeout(()=>setSaved(false),2000); };
  return (
    <div>
      ...
      <div className="panel mb-16"><div className="panel-body">
        <div className="profile-hero">
          ...
          <div className="profile-info">
            ...
            <div style={{fontSize:"0.78rem",color:"var(--text3)",marginTop:4,fontStyle:"italic"}}>{currentUser.experience||0} god. iskustva · {myBets.length} klađenja</div>
          </div>
        ...
        {editing&&<div><div className="divider"/>
      ...
      <div className="stats-grid mb-16">
        {[{val:currentUser.coins?.toLocaleString(),label:"Gold",cls:""},{val:myBets.length,label:"Klađenja",cls:""},{val:wonBets.length,label:"Pobjede",cls:"green"}].map(({val,label,cls})=>(
      ...
      <div className="panel"><div className="panel-body">
        {myBets.length===0?<div className="empty-state" style={{padding:"32px 0"}}><p>Još nisi postavio nijedan bet.</p></div>:[...myBets].reverse().map(bet=>{
          const match=matches.find(m=>m.id===bet.matchId); if(!match)return null;
          const {oddsA,oddsB}=calcOdds(match.teamA,match.teamB);
          const odds=bet.side==="A"?oddsA:oddsB, finished=match.status==="finished", won=match.winner===bet.side;
          return <div key={bet.id} className="bet-hist-row"><div><div className="bet-match">{match.title}</div><div className="bet-detail">Tim {bet.side} · {odds}x kvota · {bet.amount} G ulog</div></div><div className="bet-result">{!finished&&<span className="text-muted">U tijeku</span>}{finished&&won&&<span className="text-green">+{Math.floor(bet.amount*odds)} G ✓</span>}{finished&&!won&&<span className="text-red">-{bet.amount} G ✗</span>}</div></div>;
        })}
      </div></div>
    </div>
  );
